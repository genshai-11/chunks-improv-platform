...
export default function SetupPresenter({ onStart, nineRouterConfig, onUpdateNineRouter, theme }: SetupPresenterProps) {
  ...
  const [topic, setTopic] = useState<string>('Chủ đề tự do');
  ...
  const [showQuickLaunchModal, setShowQuickLaunchModal] = useState<boolean>(false);
  ...
  const [regenCueLang, setRegenCueLang] = useState<'vi' | 'en'>('vi');
  const [regenCueMode, setRegenCueMode] = useState<'9router'>('9router');
  const [regenCueLoading, setRegenCueLoading] = useState<boolean>(false);

  const [regenSvgCueData, setRegenSvgCueData] = useState<{ lessonId: string, cue: any, cueIndex: number } | null>(null);
  const [regenSvgLoading, setRegenSvgLoading] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (!topic || topic === 'Thế giới động vật hoang dã' || topic === 'Physical animal actions and gestures' || topic === 'Animal Sound Imitation' || topic === 'Chủ đề tự do') {
      if (activeTab === 'motion') {
        setTopic('Hành động chuyển động tự do');
      } else if (activeTab === 'sound') {
        setTopic('Mô phỏng âm thanh tự do');
      } else {
        setTopic('Chủ đề tự do');
      }
    }
  }, [activeTab]);
  ...
  const handleExecuteRegenCueAudio = async () => {
    if (!regenCueData) return;
    setRegenCueLoading(true);
    setDbStatusMsg(`⚡ Đang tạo lại audio [${regenCueLang}] cho: ${regenCueData.cue.text}...`);
    try {
      const { lessonId, cue, cueIndex } = regenCueData;
      const targetText = regenCueLang === 'vi' ? cue.text : (cue.translation || cue.text);
      ...
      const res = await fetch("/api/tts", {
        ...
        body: JSON.stringify({
          text: targetText,
          language: regenCueLang,
          ttsMode: regenCueMode,
          nineRouterConfig
        })
      });

      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Lỗi TTS server");
      
      const newBase64 = data.audio;
      
      const les = savedLessons.find(x => x.id === lessonId);
      if (les) {
          const updatedCues = [...les.cues];
          updatedCues[cueIndex] = { ...updatedCues[cueIndex], [regenCueLang === 'vi' ? 'audioVi' : 'audioEn']: newBase64 };
          ...
          if (updateRes.ok) {
              setDbStatusMsg("✅ Audio generator updated for card.");
              loadLessons();
              setRegenCueData(null);
          } else {
              throw new Error("Lưu database thất bại");
          }
      }
    } catch (err: any) {
      setDbStatusMsg(`❌ Regen error: ${err.message}`);
    } finally {
      setRegenCueLoading(false);
      setTimeout(() => setDbStatusMsg(''), 4000);
    }
  };

  const handleExecuteRegenCueSvg = async () => {
    if (!regenSvgCueData) return;
    setRegenSvgLoading(true);
    setDbStatusMsg(`⚡ Đang dùng LLM vẽ lại SVG cho thẻ: ${regenSvgCueData.cue.text}...`);
    try {
      const { lessonId, cue, cueIndex } = regenSvgCueData;
      const les = savedLessons.find(x => x.id === lessonId);
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Lỗi SVG server");
      
      const newSvg = data.svg;
      
      const updatedCues = [...les.cues];
      const fieldToUpdate = les.type === 'motion' ? 'poseJson' : 'soundText'; // Using soundText / poseJson to store SVG string directly, or should we save in a new property?
      // Let's store the raw svg in a new property `svgData`
      updatedCues[cueIndex] = { ...updatedCues[cueIndex], svgData: newSvg };
      ...
      if (updateRes.ok) {
          setDbStatusMsg("✅ SVG graphic updated for card.");
          loadLessons();
          setRegenSvgCueData(null);
      } else {
          throw new Error("Lưu database thất bại");
      }
    } catch (err: any) {
      setDbStatusMsg(`❌ Regen SVG error: ${err.message}`);
    } finally {
      setRegenSvgLoading(false);
      setTimeout(() => setDbStatusMsg(''), 4000);
    }
  };
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.cues && data.cues.length > 0) {
          ...
          setCustomCues(prev => [...prev, ...adapted]);
          // Set first newly added cue as expanded so they see details easily
          if (adapted.length > 0) {
            setExpandedCueId(adapted[0].id);
          }
          if (data.suggestedSlug) {
            setTopic(data.suggestedSlug);
          }
  ...
  return (
    <div id="setup-panel" className="max-w-6xl mx-auto py-2 px-6 h-full flex flex-col justify-center animate-fade-in">
                                  ...
                                  >
                                    <div className="flex items-center gap-1.5 truncate">
                                      ...
                                      <div className="flex gap-1 items-center shrink-0 ml-1.5">
                                        <button 
                                          title={c.audioVi ? "Nghe Audio VI" : "Chưa có Audio VI"}
                                          onClick={() => c.audioVi && new Audio(`data:audio/mp3;base64,${c.audioVi}`).play()}
                                          className={`text-[8px] font-mono select-none px-1 rounded transition-all flex items-center gap-0.5 ${
                                          c.audioVi ? 'bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 font-extrabold cursor-pointer active:scale-95' : 'bg-neutral-800 text-neutral-500 line-through cursor-not-allowed'
                                        }`}>
                                          <Volume2 className="w-2.5 h-2.5 inline" /> VI
                                        </button>
                                        <button 
                                          title={c.audioEn ? "Nghe Audio EN" : "Chưa có Audio EN"}
                                          onClick={() => c.audioEn && new Audio(`data:audio/mp3;base64,${c.audioEn}`).play()}
                                          className={`text-[8px] font-mono select-none px-1 rounded transition-all flex items-center gap-0.5 ${
                                          c.audioEn ? 'bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 font-extrabold cursor-pointer active:scale-95' : 'bg-neutral-800 text-neutral-500 line-through cursor-not-allowed'
                                        }`}>
                                          <Volume2 className="w-2.5 h-2.5 inline" /> EN
                                        </button>
                                      </div>
                                    </div>
                                    
                                    <div className="flex gap-1 items-center shrink-0">
                                      {/* Individual Audio Regenerate Modal Trigger */}
                                      <button
                                        type="button"
                                        title="Regenerate/Create Audio cho Thẻ này"
                                        onClick={() => {
                                          setRegenCueData({ lessonId: les.id, cue: c, cueIndex: i });
                                        }}
                                        className={`p-1.5 rounded-lg border text-[8px] font-bold transition-all active:scale-95 ${
                                          theme === 'black' ? 'bg-neutral-900 border-neutral-800 text-slate-400 hover:text-white' : 'bg-slate-100 border-slate-200 text-slate-500 hover:text-slate-800'
                                        }`}
                                      >
                                        <RefreshCw className="w-3 h-3" />
                                      </button>
                                      
                                      {/* Individual Regenerate Visual Trigger (SVG) */}
                                      {(cueCategory === 'motion' || cueCategory === 'sound') && (
                                        <button
                                          type="button"
                                          title={`Tạo lại Visual SVG (${cueCategory}) cho Thẻ này`}
                                          onClick={() => {
                                            setRegenSvgCueData({ lessonId: les.id, cue: c, cueIndex: i });
                                          }}
                                          className={`p-1.5 rounded-lg border text-[8px] font-bold transition-all active:scale-95 ${
                                            theme === 'black' ? 'bg-neutral-900 border-neutral-800 text-indigo-400 hover:text-indigo-300' : 'bg-indigo-50 border-indigo-200 text-indigo-500 hover:text-indigo-700'
                                          }`}
                                        >
      ...
      {/* QUICK LAUNCH MODAL */}
      {showQuickLaunchModal && quickLaunchLesson && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className={`w-full max-w-sm rounded-2xl shadow-xl border overflow-hidden ${
            ...
            <div className="p-5 space-y-5">
              <div className="space-y-1.5">
                <label className={`block uppercase text-[10px] font-black tracking-wider ${labelText}`}>Ngôn ngữ phát âm</label>
                <select
                  value={quickLaunchLanguage}
                  onChange={(e) => setQuickLaunchLanguage(e.target.value as any)}
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className={`block uppercase text-[10px] font-black tracking-wider ${labelText}`}>Giãn cách thời gian</label>
                  <span className="text-red-500 font-black text-xs">{quickLaunchDuration}s</span>
                </div>
                <input
                  type="range"
                  min="3"
                  max="7"
                  value={quickLaunchDuration}
                  onChange={(e) => setQuickLaunchDuration(Number(e.target.value))}
                  className="w-full accent-red-500 h-1 rounded-lg cursor-pointer bg-slate-700 mt-2.5"
                />
              </div>
            </div>
          ...
          <div className={`w-full max-w-sm rounded-2xl shadow-xl border overflow-hidden ${
            ...
            <div className={`px-5 py-4 border-b flex items-center justify-between ${
              ...
              <h3 className={`text-base font-bold font-display tracking-tight flex items-center gap-2 ${
                ...
                Regen Audio: Thẻ #{regenCueData.cueIndex + 1}
              </h3>
              <button 
                onClick={() => setRegenCueData(null)}
                className={`p-1.5 rounded-full hover:bg-slate-500/10 transition-colors ${
                  theme === 'black' ? 'text-slate-400 hover:text-slate-200' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
            ...
            <div className="p-5 space-y-4">
              <div className="space-y-1.5">
                ...
                <select
                  value={regenCueLang}
                  onChange={(e) => setRegenCueLang(e.target.value as any)}
                  className={`w-full px-3 py-2 border rounded-xl focus:outline-none focus:ring-1 focus:ring-emerald-500 text-xs font-bold cursor-pointer ${innerBg}`}
                >
                  <option value="vi" className={theme === 'black' ? 'bg-neutral-950' : 'bg-white'}>🇻🇳 Tiếng Việt</option>
                  <option value="en" className={theme === 'black' ? 'bg-neutral-950' : 'bg-white'}>🇬🇧 Tiếng Anh</option>
              ...
              <div className="space-y-1.5">
                <label className={`block uppercase text-[10px] font-black tracking-wider ${labelText}`}>Model/Engine sinh luồng TTS</label>
                <select
                  value={regenCueMode}
                  onChange={(e) => setRegenCueMode(e.target.value as any)}
                  className={`w-full px-3 py-2 border rounded-xl focus:outline-none focus:ring-1 focus:ring-emerald-500 text-xs font-bold cursor-pointer ${innerBg}`}
                >
                  <option value="9router" className={theme === 'black' ? 'bg-neutral-950' : 'bg-white'}>9Router Streaming API (Fast)</option>
                  <option value="gemini" className={theme === 'black' ? 'bg-neutral-950' : 'bg-white'}>Google Gemini Flash (Natural)</option>
                  <option value="local" className={theme === 'black' ? 'bg-neutral-950' : 'bg-white'}>Local OS Voices (Legacy)</option>
                </select>
              </div>
            </div>
            ...
            <div className={`p-5 border-t flex justify-end gap-3 ${
              ...
              <button
                ...
                onClick={() => setRegenCueData(null)}
                className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
                  theme === 'black' ? 'text-slate-400 hover:text-slate-200 hover:bg-neutral-800' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-200'
                }`}
              >
                Hủy
              </button>
              <button
                type="button"
                onClick={handleExecuteRegenCueAudio}
                disabled={regenCueLoading}
                className="px-6 py-2 rounded-xl text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-500 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-emerald-500/20 disabled:opacity-50"
              >
                {regenCueLoading ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <RefreshCw className="w-3.5 h-3.5" />}
                REGENERATE
              </button>
            </div>
          </div>
          ...
          <div className={`w-full max-w-sm rounded-2xl shadow-xl border overflow-hidden ${
            ...
            <div className={`px-5 py-4 border-b flex items-center justify-between ${
              ...
              <h3 className={`text-base font-bold font-display tracking-tight flex items-center gap-2 ${
                ...
                Regen SVG Card: Thẻ #{regenSvgCueData.cueIndex + 1}
              </h3>
              <button 
                onClick={() => setRegenSvgCueData(null)}
                className={`p-1.5 rounded-full hover:bg-slate-500/10 transition-colors ${
                  theme === 'black' ? 'text-slate-400 hover:text-slate-200' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
            ...
            <div className="p-5 space-y-4">
              <p className={`text-xs ${smallText} leading-relaxed`}>
                Xác nhận muốn dùng AI tự động phân tích và vẽ lại hình hoạ SVG (motion layout, âm thanh đồ hoạ) cho thẻ "<span className="font-bold">{regenSvgCueData.cue.text}</span>"? Hoạt động dùng prompt engineering & Gemini.
              </p>
            </div>
            ...
            <div className={`p-5 border-t flex justify-end gap-3 ${
              ...
              <button
                type="button"
                onClick={handleExecuteRegenCueSvg}
                disabled={regenSvgLoading}
                className="px-6 py-2 rounded-xl text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-500 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-indigo-500/20 disabled:opacity-50"
              >
                {regenSvgLoading ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <RefreshCw className="w-3.5 h-3.5" />}
                GENERATE NOW
              </button>
            </div>
          </div>
